...
export default function TwoFactorSettings() {
  ...
  // Its setState calls are all behind an awaited listFactors() call, not
  // actually synchronous, but the lint rule doesn't trace through a named
  // async function to see that.
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
      <div className="border-b border-gray-100 bg-gray-50/60 px-6 py-4">
        ...
        <p className="mt-0.5 text-xs text-gray-500">Require a code from an authenticator app when signing in.</p>
      </div>
      ...
      <div className="p-6">
        ...
        {status === 'disabled' && (
          <div className="flex items-center justify-between gap-4">
            ...
            <button onClick={startEnroll} disabled={working}
              className="flex shrink-0 items-center gap-2 rounded-xl bg-brand px-4 py-2.5 text-sm font-semibold text-white shadow-md shadow-brand/20 transition hover:bg-brand-dark disabled:opacity-50">
        ...
        {status === 'enrolling' && (
          <div className="space-y-5">
            <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-start">
              {qrCode && (
                <div className="shrink-0 rounded-2xl border border-gray-200 bg-white p-3">
                  ...
                  <img src={`data:image/svg+xml;utf-8,${encodeURIComponent(qrCode)}`} alt="Authenticator QR code" className="h-36 w-36" />
                </div>
              )}
            ...
            <form onSubmit={verifyEnroll} className="flex flex-wrap items-end gap-3">
            ...
            <button type="button" onClick={cancelEnroll} disabled={working}
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">You&apos;ll need a code from your authenticator app to sign in.</p>
                </div>
              </div>
