...
export async function POST(request: NextRequest) {
  ...
  const service = svc()
  // Same rationale as insights-batch/route.ts's unavailableFallback — this
  // handler previously had no top-level try/catch, so an RPC/network
  // exception anywhere below crashed this single tile's refresh with a bare
  // 500 instead of the same graceful "unavailable" state already used for
  // the explicit embeddings-failure/no-chunks cases just below.
  const unavailableFallback = () =>
    NextResponse.json({ insight: 'Insight temporarily unavailable.', sentiment: 'neutral' as const, sources: [], noData: true, label })
  ...
  try {
    ...
    if (!embedding) return unavailableFallback()
